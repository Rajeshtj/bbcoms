...
describe('Verify able to create a automated message', () => {
    ...
    it('Verify able to Create a data source and a basic attendance automated message', async() => {
        await WaitForExist(ImportsPageObjects.DataSourceForm);
        await ImportsCommonFunctions.CreateDataSource(ImportsTestData.DataSourceType[2]);
        let ImportName = `Auto_${ImportsTestData.MessageTypeValue.Attendance[0]}_Message`;
        await ImportsCommonFunctions.RemoveImportsData(ImportName);
        await LoginToBBcoms.navigateToPages('Messages');
        createdImportName = await ImportsCommonFunctions.CreateMessageImports(ImportsTestData.DataSourceType[2],
            ImportsTestData.MessageTypeValue.Attendance[0]);
    ...
    it('Verify able to Run the automated message import', async () => {
        let messageDisplayed = await ImportsCommonFunctions.RunImports(ImportsTestData.MessageTypeValue.Attendance[0], 'Message');
