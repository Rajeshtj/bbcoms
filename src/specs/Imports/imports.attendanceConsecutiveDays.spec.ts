...
describe('Verify able to create attendance consecutive days automated message', () => {
    ...
    it('#SMOKE Verify able to Create a data source and a attendance consecutive days automated message', async () => {
        ...
        let ImportName = `Auto_${ImportsTestData.MessageTypeValue.Attendance[4]}_Message`;
        ...
        // ImportsTestData.MessageTypeValue.Attendance[4]);
        await ImportsCommonFunctions.enterAttendanceCodes();
        await ImportsCommonFunctions.enterThreshold('1');
        await ImportsCommonFunctions.saveMessageImports();
    });

    it('#SMOKE Verify able to Schedule the automated message import', async () => {
    ...
    it('#SMOKE Verify able to Run the automated message import', async () => {
        let messageDisplayed = await ImportsCommonFunctions.RunImports(ImportsTestData.MessageTypeValue.Attendance[4], 'Message');
        expect(messageDisplayed).to.be.true;
    });

    it('#SMOKE Verify able to enter notes for the created automated message', async () => {
    ...
    it('#SMOKE Verify able to view the run logs', async () => {
