...
class ReportsFunctions {
   ...
   async ReportScheduling(){
    ...
    async WaitForReportToBeGenerated(){
        await $(ReportsPageObjects.PleaseWaitSpinner).waitForDisplayed({timeout: 180000, timeoutMsg: 'Report generation takes more than 3 minutes', reverse: true, interval:1000});
        await $(ReportsPageObjects.PleaseWaitProgress).waitForDisplayed({timeout: 180000, timeoutMsg: 'Report display progress taking more than 10 secs', reverse: true, interval:1000});
    }
    ...
    async DownloadReport(){
        ...
        await $(ReportsPageObjects.PleaseWaitSpinner).waitForDisplayed({timeout: 120000, timeoutMsg: 'Report Download takes more than 2 minutes', reverse: true, interval:1000});       // await console.log("55555555555555")
